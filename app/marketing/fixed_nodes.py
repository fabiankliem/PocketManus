"""
Fixed marketing-specific nodes for PocketFlow integration.

This module provides specialized nodes for marketing tasks, including
...
class MarketingNode(Node):
    """Base class for all marketing nodes."""
    
    def __init__(self, name="marketing_node", max_retries: int = 3, wait: int = 1):
        """Initialize a marketing node."""
...
class ResearchNode(MarketingNode):
...
class ContentGenerationNode(MarketingNode):
    ...
    def post(self, shared: Dict[str, Any], prep_res: Dict[str, Any], exec_res: str) -> Dict[str, Any]:
        ...
        shared["generated_content"] = self.result
...
class ContentOptimizationNode(MarketingNode):
...
class ChannelAdapterNode(MarketingNode):
    ...
    def __init__(self, channel: str = "website", max_retries: int = 3, wait: int = 1):
        ...
        super().__init__(name=f"channel_{channel}", max_retries=max_retries, wait=wait)
    ...
    def exec(self, prep_res: Dict[str, Any]) -> str:
        ...
        # Check if content is a string or a dictionary
        if isinstance(content, dict) and "generated_content" in content:
            content_str = content["generated_content"]
        else:
            content_str = str(content)
            
        adapted_content = f"Adapted for {self.channel}: {content_str[:100]}..."
...
class AnalyticsNode(MarketingNode):
    ...
    def __init__(self, analytics_type: str = "data_collection", max_retries: int = 3, wait: int = 1):
        ...
        super().__init__(name="analytics", max_retries=max_retries, wait=wait)
